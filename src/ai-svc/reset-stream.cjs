...
async function resetStream() {
  ...
  try {
    ...
    try {
      await jsm.streams.info('ai-pubsub');
      streamExists = true;
      console.log('\nStream exists, deleting...');
    } catch (err) {
      console.log('\nStream does not exist, will create new one');
    }

    // Delete if exists
    if (streamExists) {
      await jsm.streams.delete('ai-pubsub');
      console.log('✓ Stream deleted: ai-pubsub');
    }

    // Create stream
    console.log('\nCreating stream: ai-pubsub');
